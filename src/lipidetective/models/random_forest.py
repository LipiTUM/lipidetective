...
class RandomForest:
    ...
    def run(self):
        ...
        if self.config["random_forest"]["type"] == "single_classifier":
            ...
            prediction_statistics_single = self.calculate_accuracy(
                ...
                test_labels_lipid_names,
                "Single Classifier",
                "classification",
            )
            self.plot_decision_tree(
                single_classifier.estimators_[0], "random_forest_single_classifier_tree_1.png"
            )
            ...
            self.plot_decision_tree(
                triple_classifier_hg.estimators_[0], "random_forest_triple_classifier_hg_tree_1.png"
            )
            self.plot_decision_tree(
                triple_classifier_fa1.estimators_[0],
                "random_forest_triple_classifier_fa1_tree_1.png",
            )
            self.plot_decision_tree(
                triple_classifier_fa2.estimators_[0],
                "random_forest_triple_classifier_fa2_tree_1.png",
            )
            ...
            self.plot_decision_tree(
                triple_regressor_hg.estimators_[0], "random_forest_triple_regressor_hg_tree_1.png"
            )
            self.plot_decision_tree(
                triple_regressor_fa1.estimators_[0], "random_forest_triple_regressor_fa1_tree_1.png"
            )
            self.plot_decision_tree(
                triple_regressor_fa2.estimators_[0], "random_forest_triple_regressor_fa2_tree_1.png"
            )
    ...
    def use_triple_classifier(self, train_features, train_labels, test_features):
        ...
        prediction_output: list[list[Any]] = [
            list(item)
            for item in zip(
    ...
    def calculate_accuracy(self, prediction, labels, model, task):
        ...
        label_count_str = "\n".join(
            [" - ".join([key, str(value)]) for key, value in label_count.items()]
        )
        labels_correct_str = "\n".join(
            [" - ".join([key, str(value)]) for key, value in labels_correct.items()]
        )
    ...
    def write_output_to_file(self, statistics):
        for model, statistic in statistics.items():
            with open(
                os.path.join(
                    self.config["files"]["output"], f"random_forest_{model}_prediction_summary.txt"
                ),
                "w",
            ) as file:
    ...
    def prepare_data(self):
        ...
        if self.config["files"]["splitting_instructions"]:
            if self.config["files"]["splitting_instructions"] == "leakage":
                ...
                self.extract_info_dataset(
                    dataset_file["/lipid_classes/"],
                    splitting_instructions["val"],
                    train_set,
                    test_set,
                )
