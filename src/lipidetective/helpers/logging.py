...
class CustomAccuracy(Metric):
    ...
    def __init__(self, evaluator: Evaluator, lipid_species_names: list[str]) -> None:
        ...
        self.add_state(
            "confusion_matrix",
            default=torch.zeros((nr_lipid_species + 1, nr_lipid_species)),
            dist_reduce_fx="sum",
        )
    ...
    def update(
        ...
        if model == "transformer":
            (
                ...
                total,
                prediction_matrix,
            ) = self.evaluator.evaluate_custom_transformer_accuracy(
...
class CustomLogger(Logger):
                ...
                if self.model == "transformer":
                    ...
                    writer.writerow(
                        [
                            metrics["epoch"],
                            metrics["train_loss_epoch"],
                            metrics["customaccuracy_train_accuracy_epoch"],
                            r2_score,
                            metrics["train_mae_hg_epoch"],
                            metrics["train_mae_fa1_epoch"],
                            metrics["train_mae_fa2_epoch"],
                        ]
                    )
                ...
                if self.model == "transformer":
                    writer.writerow(
                        [
                            ...
                            "{:.2f}".format(
                                metrics["customaccuracy_val_mean_accuracy_epoch"] * 100
                            ),
                        ]
                    )
                else:
                    ...
                    writer.writerow(
                        [
                            metrics["epoch"],
                            metrics["val_loss_epoch"],
                            metrics["customaccuracy_val_accuracy_epoch"],
                            r2_score,
                            metrics["val_mae_hg_epoch"],
                            metrics["val_mae_fa1_epoch"],
                            metrics["val_mae_fa2_epoch"],
                        ]
                    )
            ...
            with open(self.test_csv_path, "a") as f:
                ...
                if self.model == "transformer":
                    writer.writerow(
                        [
                            step,
                            metrics["test_loss_step"],
                            "{:.2f}".format(metrics["customaccuracy_test_accuracy_step"] * 100),
                            "{:.2f}".format(
                                metrics["customaccuracy_test_mean_accuracy_step"] * 100
                            ),
                        ]
                    )
    ...
    def transform_token_predictions_to_string(
        ...
        for row in preds_vs_labels:
            ...
            batch = row[1]
            prediction = "".join(
                self.librarian.translate_tokens_to_name(row[2 : self.output_seq_length + 2])
            ).replace("<PAD>", "")
            label = "".join(
                self.librarian.translate_tokens_to_name(
    ...
    def transform_test_token_predictions_to_string(
        ...
        for row in preds_vs_labels:
            prediction = "".join(
                self.librarian.translate_tokens_to_name(row[0 : self.output_seq_length])
            ).replace("<PAD>", "")
            label = "".join(
                self.librarian.translate_tokens_to_name(
                    row[self.output_seq_length : (2 * self.output_seq_length)]
                )
            ).replace("<PAD>", "")
    ...
    def save_lipid_wise_metrics(
        ...
        if train_confusion_matrix is not None and train_lipids is not None:
            ...
            train_metrics_df.to_csv(os.path.join(self.save_path, "train_lipid_metrics.csv"))
        ...
        if val_confusion_matrix is not None and val_lipids is not None:
            ...
            val_metrics_df.to_csv(os.path.join(self.save_path, "val_lipid_metrics.csv"))
        ...
        if test_confusion_matrix is not None and test_lipids is not None:
            ...
            test_metrics_df.to_csv(os.path.join(self.save_path, "test_lipid_metrics.csv"))
    ...
    def calculate_lipid_metrics(self, confusion_matrix: pd.DataFrame) -> pd.DataFrame:
        ...
        for lipid in confusion_matrix.columns[:-1]:
            ...
            calculated_scores.append(
                {"lipid": lipid, "precision": precision, "recall": recall, "f1": f1}
            )
    ...
    def plot_loss_and_accuracy(self, df: pd.DataFrame, workflow: str, output_folder: str) -> None:
        plot_name = (
            ...
            if self.fold != "."
            else f"plot_loss_accuracy_{workflow}.png"
        )
        plot_name_loss = (
        ...
        ax2 = cast(Axes, plt.twinx())
        sns.lineplot(data=df.accuracy, color="b", ax=ax2)
        plt.ylabel("Accuracy in %", fontsize=14)
        y_ticks = list(range(0, 101, 10))
        ax2.set_yticks(y_ticks)
        ax2.legend(["Accuracy"], loc=(1.15, 0.95), frameon=False, fontsize=13)

        plt.title("Loss and Accuracy per Epoch", fontsize=16)
    ...
    def plot_loss_and_both_accuracies(
        ...
        plot_name = (
            ...
            if self.fold != "."
            else f"plot_loss_accuracies_{workflow}.png"
        )
        plot_name_loss_custom_accuracy = (
            f"plot_loss_custom_accuracy_{workflow}_{self.fold}.png"
            if self.fold != "."
            else f"plot_loss_custom_accuracy_{workflow}.png"
        )
        ...
        ax1.set_yscale("log")
        plt.xlabel("Epoch", fontsize=14)
        ax1.legend(["Loss"], loc=(1.1, 0.92), frameon=False, fontsize=13)
        ...
        y_ticks = list(range(0, 101, 10))
        ax2.set_yticks(y_ticks)
        ax2.legend(["Mean Accuracy / Lipid"], loc=(1.1, 0.95), frameon=False, fontsize=13)
        ...
        ax2.set_yticks(y_ticks)
        ax2.legend(
            ["Mean Accuracy / Lipid", "_", "Accuracy"], loc=(1.1, 0.95), frameon=False, fontsize=13
        )
        plt.tight_layout(w_pad=4)
    ...
    def plot_loss_and_mae(self, df: pd.DataFrame, workflow: str, output_folder: str) -> None:
        plot_name = (
            ...
            if self.fold != "."
            else f"plot_loss_mae_{workflow}.png"
        )
        plot_name_mae = (
            f"plot_mae_{workflow}_{self.fold}.png"
            if self.fold != "."
            else f"plot_mae_{workflow}.png"
        )
        ...
        ax1.legend(
            ["mae_hg", "_", "mae_fa1", "_", "mae_fa2"], loc=(1.15, 0.95), frameon=False, fontsize=13
        )
        ...
        ax2.set_yscale("log")
        plt.xlabel("Epoch", fontsize=14)
        ax2.legend(["Loss"], loc=(1.15, 0.92), frameon=False, fontsize=13)

        plt.title("Loss and MAE per Epoch", fontsize=16)
        plt.tight_layout(w_pad=4)
        plt.savefig(os.path.join(output_folder, plot_name), dpi=300)
        plt.close(figure)
    ...
    def plot_loss_and_r2(self, df: pd.DataFrame, workflow: str, output_folder: str) -> None:
        plot_name = (
            ...
            if self.fold != "."
            else f"plot_loss_r2_{workflow}.png"
        )
        df.index = df.epoch
        ...
        ax2.set_ylim(top=3)
        ax2.legend(["R2"], loc=(1.15, 0.95), frameon=False, fontsize=13)

        plt.title("Loss and R2 per Epoch", fontsize=16)
        plt.tight_layout(w_pad=4)
        plt.savefig(os.path.join(output_folder, plot_name), dpi=300)
        plt.close(figure)
    ...
    def plot_confusion_matrix_as_heatmap(
        ...
        sns.heatmap(
            confusion_matrix.drop("sum_predictions", axis=1),
            cmap="rocket_r",
            fmt="g",
            annot=True,
            linewidth=0.5,
...
class PredictionLogger(Logger):
    def __init__(self, save_dir: str, log_every_n_steps: int, config: dict[str, Any]) -> None:
        ...
        if "predict" in config:
            self.save_top3 = (
                config["predict"]["output"] == "top3" if "output" in config["predict"] else False
            )
            self.save_spectrum = (
                config["predict"]["save_spectrum"]
                if "save_spectrum" in config["predict"]
                else False
            )
            self.keep_empty = (
                config["predict"]["keep_empty"] if "keep_empty" in config["predict"] else False
            )
            self.keep_wrong_polarity_preds = (
                config["predict"]["keep_wrong_polarity_preds"]
                if "keep_wrong_polarity_preds" in config["predict"]
                else False
            )
            self.confidence_threshold = (
                config["predict"]["confidence_threshold"]
                if "confidence_threshold" in config["predict"]
                else 0
            )
        else:
    ...
    def generate_output_files(self) -> tuple[str, str | None]:
        ...
        with open(predictions_path, "w") as f:
            writer = csv.writer(f)
            writer.writerow(
                ["file", "polarity", "spectrum_index", "precursor", "prediction", "confidence"]
            )

        if self.save_top3:
            top_3_predictions_path = os.path.join(self.output_directory, "top3_predictions.csv")
    ...
    def transform_test_token_predictions_to_string(
        ...
        for i in range(len(tokens)):
            ...
            if not isinstance(spectrum_index, str):
            ...
            for j in range(3):
                ...
                prediction = (
                    "".join(self.librarian.translate_tokens_to_name(tokens_temp))
                    .replace("<PAD>", "")
                    .replace("<EOS>", "")
                )
            ...
            top_prediction = spectrum_predictions[0][0]
            top_confidence = spectrum_predictions[0][1]
            translated_tokens_row.extend([top_prediction, top_confidence])

            polarity_prediction = str(top_prediction)[-1] if top_prediction else polarity

            if float(top_confidence) > self.confidence_threshold:
        ...
        translated_tokens_arr = np.array(translated_tokens)
        translated_tokens_top_3_arr = np.array(translated_tokens_top_3)
