...
class LipidLibrary:
    def __init__(self):
        ...
        self.molecular_lipid_species = read_yaml(
            os.path.join(cwd, "lipid_info/molecular_lipid_species.yaml")
        )
        ...
        self.max_mass_side_chains = (
            max(self.side_chains_to_mass.values()) - self.mean_mass_side_chains
        )
        ...
        self.headgroups_mass_norm = {
            key: ((value - self.mean_mass_headgroup) / self.max_mass_headgroup)
            for key, value in self.headgroups.items()
        }
        self.side_chains_mass_norm = {
            key: ((value - self.mean_mass_side_chains) / self.max_mass_side_chains)
            for key, value in self.side_chains_to_mass.items()
        }
    ...
    def get_transformer_label(self, lipid_species: str, adduct: str, output_seq_length: int):
        if lipid_species:
            ...
            token_ids = (
                [self.tokens["<SOS>"]] + name_tokens + [adduct_tokens] + [self.tokens["<EOS>"]]
            )
        else:
            lipid_name_components = [""]
            token_ids = [self.tokens["<SOS>"], self.tokens["<EOS>"]]

        token_tensor: torch.Tensor = torch.IntTensor(token_ids)
    ...
    def custom_accuracy_scoring(self, prediction, label):
        (
            ...
            pred_func_group_2,
            pred_func_group_3,
        ) = self.get_lipid_species_components(prediction)
        (
            label_class,
            label_fa,
            label_bond1,
            ...
            label_func_group_2,
            label_func_group_3,
        ) = self.get_lipid_species_components(label)
