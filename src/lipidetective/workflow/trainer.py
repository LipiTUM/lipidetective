...
class Trainer:
        ...
        if self.config["model"] != "random_forest":
            ...
            if torch.cuda.is_available():
                ...
                if self.config["cuda"]["gpu_nr"]:
                    ...
                    logging.info(
                        f"Cuda has been found, running with the following devices: {self.devices}"
                    )
                else:
                    self.devices = "auto"
            ...
            self.nr_workers = (
                self.config["training"]["nr_workers"]
    ...
    def train_with_validation(self) -> None:
        ...
        for fold in range(len(trainsets)):
            ...
            tb_logger = pl_loggers.TensorBoardLogger(
                save_dir=self.output_folder, name="tb_logger", version=version
            )
            csv_logger = pl_loggers.CSVLogger(
                save_dir=self.output_folder, name="csv_logger", version=version
            )
            ...
            if "wandb" in self.config:
                wandb.init(
                    project="lipidetective", config=self.config, group=group, name=wandb_name
                )
    ...
    def train_without_validation(self) -> None:
        ...
        custom_logger = CustomLogger(
            ...
            config=self.config,
            do_training=True,
            trainset_names=copy.deepcopy(dataset_names),
            trainset_lipids=dataset_lipids,
        )

        if "wandb" in self.config:
            group = self.config["wandb"]["group"] if self.config["wandb"]["group"] else None
            wandb.init(
                project="lipidetective", config=self.config, name=self.time_initialized, group=group
            )
    ...
    def test(self) -> None:
        ...
        custom_logger = CustomLogger(
            ...
            config=self.config,
            do_testing=True,
            testset_names=copy.deepcopy(dataset_names),
            testset_lipids=dataset_lipids,
        )
    ...
    def predict(self) -> None:
        ...
        pred_logger = PredictionLogger(
            save_dir=self.output_folder, log_every_n_steps=1, config=self.config
        )
    ...
    def schedule_tuning(self) -> None:
        ...
        val_loader = DataLoader(
            valsets[0], batch_size=self.config["training"]["batch"], num_workers=self.nr_workers
        )

        scheduler = ModifiedASHAScheduler(
            max_t=self.config["training"]["epochs"],
            grace_period=self.config["tune"]["grace_period"],
            reduction_factor=4,
        )

        self.prepare_tune_config()

        callbacks: list[LoggerCallback] = [CustomLoggerCallback(self.output_folder)]
        if "wandb" in self.config:
    ...
    def perform_data_split(self) -> DataSplit:
        ...
        if self.config["files"]["val_input"]:
            ...
            ]  # wrapping it in list so it works independent on if the data is split by fold or not
    ...
    def split_data_by_lipid_species(self, dataset_names: list[str]) -> DataSplit:
        ...
        """
        possible_lipid_species = list({name.split(" | ")[0] for name in dataset_names})
        nr_folds = self.config["training"]["k"]

        number_lipids_excluded = int(len(possible_lipid_species) / nr_folds)

        trainset_names: list[list[str]] = []
        valset_names: list[list[str]] = []
